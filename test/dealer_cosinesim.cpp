...
int main() {
    std::cout << "Dealer for CosineSimilarityLayer starting..." << std::endl;
    ...
    cosinesim_layer.generate_randomness(p0_buf, p1_buf);
    ...
    std::ofstream p0_file("./randomness/P0/cosinesim_randomness.bin", std::ios::binary);
    p0_file.write(reinterpret_cast<const char*>(p0_data), total_size);
    p0_file.close();

    std::ofstream p1_file("./randomness/P1/cosinesim_randomness.bin", std::ios::binary);
